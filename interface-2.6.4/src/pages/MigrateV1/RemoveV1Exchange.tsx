    ...
    exchangeContract
      ...
      .then((response: TransactionResponse) => {
        ...
        addTransaction(response, {
          summary: `Remove ${chainId && token.equals(WETH[chainId]) ? 'WPLS' : token.symbol}/ETH V1 liquidity`
        })
  ...
  return (
    <AutoColumn gap="20px">
      ...
      <TYPE.darkGray style={{ textAlign: 'center' }}>
        {`Your Uniswap V1 ${
          chainId && token.equals(WETH[chainId]) ? 'WPLS' : token.symbol
  ...
  const liquidityToken: Token | undefined = useMemo(
    () =>
      validatedAddress && chainId && token
        ? new Token(chainId, validatedAddress, 18, `DexTop-V1-${token.symbol}`, 'DexTop V1')
        : undefined,
